"""
Renderfarm 3ds Max Submitter v1.0.0
Submit 3ds Max render jobs to Renderfarm directly from 3ds Max.

Installation:
  1. Copy this file to:
       %APPDATA%\Autodesk\3dsMax\<version>\ENU\scripts\startup\renderfarm_submitter.py
     OR place it anywhere and run it from the MAXScript Listener:
       python.ExecuteFile @"C:\path\to\renderfarm_submitter.py"
  2. A "Renderfarm" button will appear in the toolbar, or call:
       python.Execute "import renderfarm_submitter; renderfarm_submitter.show()"

Requirements: 3ds Max 2022+ (ships Python 3.9+). PySide2 available via qtmax.
"""
...
# PySide2 is available in 3ds Max 2022+ via qtmax or directly
try:
...
MAX_VERSIONS = [
    ("3dsmax-2025", "3ds Max 2025"),
    ("3dsmax-2024", "3ds Max 2024"),
    ("3dsmax-2023", "3ds Max 2023"),
    ("3dsmax-2022", "3ds Max 2022"),
]

RENDERERS = [
    ("arnold",  "Arnold (MAXtoA)"),
    ("vray",    "V-Ray"),
    ("corona",  "Corona"),
    ("scanline","Scanline"),
    ("artrender","ART Renderer"),
]
    ...
    def do_GET(self):
        ...
        if parsed.path == "/callback" and token and email:
            html = (
                ...
                b"<body><div class='card'><h2 style='color:#22d3ee'>&#10003; 3ds Max Connected</h2>"
                b"<p>You can close this tab and return to 3ds Max.</p></div></body></html>"
            )
...
def _scan_dependencies():
    """Collect all external file references from the 3ds Max scene."""
    if not _IN_MAX:
        return []
    seen   = set()
    assets = []
    ...
    # All file dependencies via MAXScript
    try:
        deps = rt.getFileDependencies()
        if deps:
            for dep in deps:
                _add(str(dep), "file")
    except Exception:
    ...
    # BitmapTexture nodes (more reliable than getFileDependencies for textures)
    try:
        bitmaps = rt.getClassInstances(rt.BitmapTexture)
        for bmp in bitmaps:
            try:
                fname = str(bmp.filename)
                _add(fname, "texture")
            except Exception:
                pass
    except Exception:
    ...
    # VRayBitmap (V-Ray)
    try:
        vray_bmps = rt.getClassInstances(rt.VRayBitmap)
        for vbmp in vray_bmps:
            try:
                _add(str(vbmp.HDRIMapName), "texture")
            except Exception:
                pass
    except Exception:
        pass
    ...
    # Alembic references
    try:
        abc_nodes = rt.getClassInstances(rt.AlembicContainer)
        for node in abc_nodes:
            try:
                _add(str(node.filepath), "cache_alembic")
            except Exception:
                pass
    except Exception:
        pass
    ...
    # Point cache
    try:
        pc_mods = rt.getClassInstances(rt.Point_Cache)
        for mod in pc_mods:
            try:
                _add(str(mod.filename), "cache_pointcache")
            except Exception:
                pass
    except Exception:
        pass
...
class SubmitWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            scene_file = ""
            if _IN_MAX:
                fname = str(rt.maxFileName)
                fpath = str(rt.maxFilePath)
                if fname and fpath:
                    scene_file = os.path.join(fpath, fname)
...
class RenderfarmSubmitter(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Renderfarm Submitter — 3ds Max")
    ...
    def _build_ui(self):
        ...
        self._title_edit = QtWidgets.QLineEdit("3ds Max Render")
        ...
        for val, lbl in MAX_VERSIONS:
            self._software_combo.addItem(lbl, val)
        form.addRow("Max Version:", self._software_combo)
        ...
        self._frames_edit = QtWidgets.QLineEdit("0-100")
        ...
        if _IN_MAX:
            ...
            self._title_edit.setText(f"3ds Max Render — {name}" if name else "3ds Max Render")
            try:
                f_start = int(rt.animationRange.start)
                f_end   = int(rt.animationRange.end)
                self._frames_edit.setText(f"{f_start}-{f_end}")
            except Exception:
                pass
    ...
    def _do_submit(self):
        ...
        params = {
            "token":         self._token,
            "title":         self._title_edit.text().strip() or "3ds Max Render",
            "frames":        self._frames_edit.text().strip() or "0-0",
...
def show():
    """Call from MAXScript or a toolbar button to open the submitter."""
    global _window
    parent = None
    if _HAS_QTMAX:
        try:
...
# Auto-show when run directly via python.ExecuteFile in 3ds Max
if _IN_MAX:
    show()
